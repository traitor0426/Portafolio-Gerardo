class Vista {
    ...
    async procesar_click() {
        ...
        try {
            ...
            if (!result.ok) {
                alert("Error al subir el archivo: " + result.mensaje);
                return;
            }

            // Una vez subido el archivo, enviar los datos del formulario
            const datos = this.recuperar();
            datos.archivo = result.url; // Asumimos que el servidor devuelve la URL del archivo subido
