class Vista {
    constructor(reset = false) {
        if (reset == true) {
            ...
            formContainer.classList.add("login-container");

            // Título del formulario
            const titulo = document.createElement("h2");
            titulo.innerText = "Consulta por Nombre Completo";
            formContainer.appendChild(titulo);

            // Campo de entrada para el nombre
            formContainer.append(this.input({ id: "nombre", type: "text", placeholder: "Escriba su nombre completo", class: ["input-field"] }));
            this.nombre = formContainer.querySelector("#nombre");

            // Botón de búsqueda
            formContainer.append(this.input({ id: "btnBuscar", type: "submit", value: "Buscar nombre", class: ["boton"] }));
            this.btnBuscar = formContainer.querySelector("#btnBuscar");
            this.btnBuscar.addEventListener("click", (e) => this.btnBuscar_click(e));

            // Añadir el contenedor del formulario al main
    ...
    crearTabla() {
        ...
        if (!document.querySelector("#tablaConsultas")) {
            ...
            const headers = ["Nombre", "Usuario", "Foto"];
        ...
        datos.forEach(dato => {
            ...
            for (const key in dato) {
                ...
                if (key === 'foto') {
                    ...
                    img.src = dato[key];
                    img.alt = 'Foto';
                    img.width = 50; // Ajustar el tamaño según sea necesario
                    img.height = 50; // Ajustar el tamaño según sea necesario
                    td.appendChild(img);
                } else {
